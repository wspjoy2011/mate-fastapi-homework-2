...
def test_get_movie_by_id_fields_match_database(client, db_session, seed_database):
    ...
    expected_genres = [{"id": genre.id, "name": genre.name} for genre in random_movie.genres]
    assert response_data["genres"] == expected_genres, "Genres do not match."

    expected_actors = [{"id": actor.id, "name": actor.name} for actor in random_movie.actors]
    assert response_data["actors"] == expected_actors, "Actors do not match."

    expected_languages = [{"id": lang.id, "name": lang.name} for lang in random_movie.languages]
    assert response_data["languages"] == expected_languages, "Languages do not match."
